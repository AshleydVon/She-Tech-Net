...
const { User, Course, Mentorships, Jobs, Events } = require('../models'); // Importing all models
...
const GOOGLE_CLIENT_ID = '89152183018-7bvcajfr8lo9ba9c3c6dujj7atdbf77c.apps.googleusercontent.com'; // Google Calendar Client ID

db.once('open', async () => {
  // Clean out the previous data from collections
  await cleanDB('Courses', 'courses');
  await cleanDB('Users', 'users');
  await cleanDB('Mentorships', 'mentors'); // For mentors
  await cleanDB('Jobs', 'jobs');       // For jobs
  await cleanDB('Events', 'events');   // For events
  ...
  // Fetch and Seed Courses from Courseca API
  try {
  ...
  const mentorships = await Mentorships.insertMany([
    {
      user: users[0]._id, // Pamela Washington
      expertise: ['JavaScript', 'Node.js'],
      industry: 'Software Development',  // Added industry
      yearsOfExperience: 5,             // Added years of experience
      availableTimeSlots: ['Monday 10am-12pm', 'Wednesday 2pm-4pm'],
    },
    {
      user: users[1]._id, // Elijah Holt
      expertise: ['HTML', 'CSS'],
      industry: 'Web Development',       // Added industry
      yearsOfExperience: 2,             // Added years of experience
  ...
  const jobs = await Job.insertMany([
    {
      ...
      isWomenFriendly: true,            // Filter for women-friendly companies
      supportsDiversity: true,          // Filter for diversity-supporting companies
    },
    {
      company: 'Innovatech',
  ...
  // Seed Events from Google Calendar API
  try {
    ...
    const eventsData = calendarResponse.data.items.map(event => ({
      ...
      date: event.start.dateTime || event.start.date,  // Depending on the format
      registrationLink: event.htmlLink,
      tags: ['Tech', 'Workshop'],  // Default tags, update as needed
    }));
